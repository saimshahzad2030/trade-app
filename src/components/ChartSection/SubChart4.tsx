...
import { WACCData } from "./SubChartParent";
import { mockData } from "../Searchbar/CompanySectionSearchbar";
import { PlusIcon, X } from "lucide-react";
import RoundLoader from "../Loader/RoundLoader";
import { searchStock } from "@/services/search.services";
import { getRevenueGrowthMetrices, getWaccMetrices } from "@/services/stocksFinancialMetrics.services";
...
type Props = {
  waccData: { wacc: WACCData[]; symbol: string }[];
  handleAddWACC: (newData: { wacc: WACCData[]; symbol: string }) => void;
};

const SubChart4 = ({ waccData, handleAddWACC }: Props) => {
   const [showModal, setShowModal] = React.useState(false);
    ...
    const allDates = Array.from(
      new Set(waccData.flatMap(company => company.wacc.map(entry => entry.date)))
    ).sort();
  const averageGrowth = allDates.map((date) => {
  const values = waccData
    .map(c => c.wacc.find(g => g.date === date)?.wacc)
  ...
  return +(sum / values.length).toFixed(2);
    ...
    const colors = generateColorPalette(waccData.length);
  
    const companySeries = waccData.map((company, index) => {
      const dateToPE: Record<string, number> = {};
      company.wacc.forEach(entry => {
        const parsed = parseFloat(String(entry.wacc).replace(/[$,]/g, ""));
        if (!isNaN(parsed)) {
...
,
    legend: {
      type: "scroll",
       data: [...waccData.map(c => c.symbol.toUpperCase()) ],
                    ...
                    {filteredData.map((item, index) => (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const response = await getWaccMetrices(item.symbol);
                          if (typeof response.data === "object" && "wacc" in response.data) {
                             handleAddWACC(response.data as {wacc:WACCData[],symbol:string}) 
