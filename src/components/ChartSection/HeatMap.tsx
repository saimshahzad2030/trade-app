...
// import { getSectorHeatMapData } from "@/services/sectors.services";
// import { SectorHeatmapResponse } from "@/types/types";
...
//   const [data,setData] = React.useState<SectorHeatmapResponse | null>(null)
//     React.useEffect(()=>{
//           const fetchChartData = async()=>{
//             setLoading(true)
//              let response2 = await getSectorHeatMapData();
...
const convertToTreemap = (sectorData: SectorHeatmapResponse) => {
  return Object.entries(sectorData).map(([sector, stocks]) => ({
    name: sector,
    children: Array.isArray(stocks)
      ? stocks.map(stock => {
          const percent = parseFloat(stock.percentChange.replace(/%/g, "")) || 0;
          return {
            name: `${stock.ticker} ${stock.percentChange}`,
            value: parseFloat(stock.marketCap.replace(/[$,]/g, "")) || 1,
            change: percent,
            itemStyle: { color: getColor(percent) },
            label: { formatter: stock.ticker }
          };
        })
      : []
  }));
...
export const MarketMapAll = () => {
  ...
  React.useEffect(() => {
    const fetchChartData = async () => {
      setLoading(true);
      const response = await getSectorHeatMapData();
      setData(response.data.sectorHeatmap); 
      setLoading(false);
    };
    fetchChartData();
  }, []);

  const treeData = data ? convertToTreemap(data) : [];
    ...
    <div className="grid grid-cols-4 gap-0 w-full h-full">
      {loading ? (
        Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="w-full h-[400px] p-2">
            <SkeletonLoader className="w-full h-full bg-[#2a2a3d] rounded-md" />
          </div>
        ))
      ) : (
        treeData.map((sector, i) => (
          <div key={i} className="w-full h-[400px] p-2">
            <SectorHeatmap title={sector.name} children={sector.children} />
          </div>
        ))
      )}
    </div>
