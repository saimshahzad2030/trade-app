...
//   // Calculate average WACC per year
...
import React from 'react';
import ReactECharts from 'echarts-for-react';
import { RevenueResponseType, WaccResponseType, Stock } from '@/types/types';
import { get5YearRevenue, get5YearRoc, get5YearWacc } from '@/services/stockScreening.services';
...
React.useEffect(() => {
  const fetchAllStockData = async () => {
    try {
      ...
      const res = await get5YearWacc(symbols);
   ...
   const cleanedEntries = entries.map((entry: { year: string; wacc: string }) => {
  const raw = entry.wacc.replace('%', '').trim(); // Remove % only
  const numeric = parseFloat(raw); // Direct conversion to number

  return {
    year: entry.year,
    wacc: numeric,
    display: entry.wacc, // Keep the original for tooltip
  };
});
