// "use client";
// import { Holdings, HoldingSummary, PortfolioDataResponse, Transactions } from "@/types/types";
...
// }
// const HoldingsTable = ({summaryData}:props) => {
...
import TransactionTables from "./TransactionTables";
import { textColor } from "@/utils/functionalUtils";
import { PortfolioDataResponse, Transactions } from "@/types/types";  
...
const HoldingsTable = ({ summaryData, isLoading = false }: Props) => {
...
const holdings: PortfolioDataResponse = summaryData;
          ...
          ? Array.from({ length: 5 }).map((_, i) => (
              <TableRow key={i}>
                {Array.from({ length: 13 }).map((_, j) => (
                  <TableCell key={j}>
                    <SkeletonLoader className="h-10 w-12" />
                  </TableCell>
                ))}
              </TableRow>
            ))
                  ...
                  <TableRow>
                    <TableCell colSpan={13} className="bg-[#13131f] px-4 py-3">
                      <div className="text-sm text-white">
                        ...
                        {selectedTab === "lots" && (
                          <LotTable lots={lots} holding={stock.holdings} setLots={setLots} />
                        )}
                        {selectedTab === "transactions" && (
                          <TransactionTables
                            transactions={transactions}
