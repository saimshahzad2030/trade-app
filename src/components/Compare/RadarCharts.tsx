...
const RadarCharts = ({ stocks }: Props) => {
  const allIndicators = [
    { name: "ROC", key: "roc", max: 30 },
    { name: "PE", key: "priceToEarnings", max: 100 },
    { name: "Gross Margin", key: "grossProfitMarginIndicator", max: 80 },
    { name: "WACC", key: "wacc", max: 15 },
    { name: "D/E Ratio", key: "debtToEquityRatioIndicator", max: 3 },
    { name: "Beta", key: "beta", max: 2 },
    { name: "DCF", key: "dcf", max: 300 },
    { name: "FCFF", key: "fcff", max: 150 },
  ];
  ...
  const normalizedStockData = useMemo(() => {
    return stocks.map((stock) => {
      const values = allIndicators.map((ind) => {
       let raw = (stock as any)[ind?.key];
      ...
      const normalized = selectedIndicators.map((ind) => {
        const indIndex = allIndicators.findIndex((a) => a.name === ind.value);
        const val = values[indIndex];
        const max = allIndicators[indIndex].max;
    ...
    tooltip: {
      formatter: (params: any) => {
        ...
        const values = params.value
          .map((val: number, idx: number) => `${radarIndicators[idx].name}: ${val.toFixed(1)}%`)
          .join("<br/>");
        return `<strong>${label}</strong><br/>${values}`;
      },
    },
    legend: {
