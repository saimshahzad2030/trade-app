...
import SkeletonText from "../Loader/SkeletonTextLoader";
...
const CompanyDetails = ({loading,metaData  }: CompanyDetailsProps) => {
 ...
 <p>{`${metaData?.meta.exchange} - ${metaData?.meta.currency}`}</p>
      ...
      <div className="w-full flex flex-row items-center mt-2">
        {loading ? (
  <SkeletonLoader className="bg-gray-700 h-[20px] w-40 my-2 mb-4" />
) : (
 <h1 className="text-3xl font-bold">{metaData?.meta.companyName}</h1>
      ...
      <div className="w-full flex flex-row items-center">
        <div className="w-5/12 flex flex-col items-start">
          <div className="w-full flex flex-row items-center">
             {loading ? (
  <SkeletonLoader className="bg-gray-700 h-[35px] w-36" />
) : (
<h1 className="text-4xl font-bold">
      ...
      {loading ? (
  <SkeletonLoader className="bg-gray-700 h-[16px] w-32 ml-4" />
) : (
 <p
          ...
          <div>
             {loading ? (
  <SkeletonLoader className="bg-gray-700 h-[16px] w-44 mt-2" />
) : (
